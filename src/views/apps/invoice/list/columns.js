  ...
  if (false) {
    ...
    return (
      <Avatar
        color={color}
        className="mr-50"
        content={row.user ? row.user.username : "John Doe"}
        initials
      />
    );
  }
...
export const columns = [
  ...
  //           <br />
  //           <span className="font-weight-bold">Fecha:</span> {row.fecha}
  //         </UncontrolledTooltip>
  //       </Fragment>
  //     );
  //   },
  // },
  {
    ...
    sortable: true,
    cell: (row) => {
      const name = row.user ? row.user.username : "John Doe",
        email = row.user ? row.user.email : "johnDoe@email.com";
      return (
  ...
  },
  {
    ...
    sortable: true,
    minWidth: "200px",
    cell: (row) => dayjs(row.fecha).format("DD-MM-YYYY"),
  },
  {
    name: "Action",
    minWidth: "110px",
